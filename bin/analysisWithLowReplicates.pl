...
( $repsA >= 2 ) or die "For condition A, must provide a pooled data file and at least 2 replicate files.";
( $repsB >= 2 ) or die "For condition B, must provide a pooled data file and at least 2 replicate files.";
...
my @repsHeader;
my $counter = 1;
foreach(@repsFileNames[0 .. $repsA-1]) {
	$_ = '#Condition A replicate ' . $counter . ': ' . $_;
	push @repsHeader, "A$counter-IRratio";
	$counter++;
}
$counter = 1;
foreach(@repsFileNames[$repsA .. scalar @repsFileNames - 1]) {
	$_ = '#Condition B replicate ' . $counter . ': ' . $_;
	push @repsHeader, "B$counter-IRratio";
	$counter++;
}

print "#Condition A combined: $poolAname\n";
print join("\n",@repsFileNames[0 .. $repsA-1]), "\n";
print "#Condition B combined: $poolBname\n";
...
while(<$poolA>) {
  ...
  my $ok = ($pA[20] eq "-" || $pB[20] eq "-") && ($pA[8] >= 1 || $pB[8] >= 1) && ($pA[19] >= 0.01 || $pB[19] >= 0.01) && separatedAB(\@repsIR, $repsA, $repsB);
